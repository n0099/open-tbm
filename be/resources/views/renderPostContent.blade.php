...
$getImageUrl = static fn(string $urlFilename) =>
    Regex::match('/^(?:[0-9a-f]{40}|[0-9a-f]{24})$/', $urlFilename)->hasMatch()
        ? "https://imgsrc.baidu.com/forum/pic/item/$urlFilename.jpg"
        : $urlFilename;
    ...
    @switch ($item->getType())
        ...
        @case (1)
            ...
            --}}
                ...
                ?>
            <a href="{{ empty($skipCheckUrl) ? $item->getLink() : $skipCheckUrl }}"
               target="_blank">{{ $item->getText() }}</a>
            @break
        @case (2)
            {{--表情 {"c": "滑稽", "text": "image_emoticon25", "type": "2"} --}}
                <?php
                $emoticonsIndex = [
                    'image_emoticon' => ['class' => 'client', 'type' => 'png'],
                    // 泡泡(<51)/客户端新版表情(>61)
                    // 'image_emoticon' => ['class' => 'face', 'prefix' => 'i_f', 'type' => 'gif'], // 旧版泡泡
                    'image_emoticon>51' => ['class' => 'face', 'prefix' => 'i_f', 'type' => 'gif'],
                    // 泡泡-贴吧十周年(51>=i<=61)
                    'bearchildren_' => ['class' => 'bearchildren', 'type' => 'gif'],
                    // 贴吧熊孩子
                    'tiexing_' => ['class' => 'tiexing', 'type' => 'gif'],
                    // 痒小贱
                    'ali_' => ['class' => 'ali', 'type' => 'gif'],
                    // 阿狸
                    'llb_' => ['class' => 'luoluobu', 'type' => 'gif'],
                    // 罗罗布
                    'b' => ['class' => 'qpx_n', 'type' => 'gif'],
                    // 气泡熊
                    'xyj_' => ['class' => 'xyj', 'type' => 'gif'],
                    // 小幺鸡
                    'ltn_' => ['class' => 'lt', 'type' => 'gif'],
                    // 冷兔
                    'bfmn_' => ['class' => 'bfmn', 'type' => 'gif'],
                    // 白发魔女
                    'pczxh_' => ['class' => 'zxh', 'type' => 'gif'],
                    // 张小盒
                    't_' => ['class' => 'tsj', 'type' => 'gif'],
                    // 兔斯基
                    'wdj_' => ['class' => 'wdj', 'type' => 'png'],
                    // 豌豆荚
                    'lxs_' => ['class' => 'lxs', 'type' => 'gif'],
                    // 冷先森
                    'B_' => ['class' => 'bobo', 'type' => 'gif'],
                    // 波波
                    'yz_' => ['class' => 'shadow', 'type' => 'gif'],
                    // 影子
                    'w_' => ['class' => 'ldw', 'type' => 'gif'],
                    // 绿豆蛙
                    '10th_' => ['class' => '10th', 'type' => 'gif'],
                    // 贴吧十周年
                ];
        ...
        @case (4)
            ...
            <a href="http://tieba.baidu.com/home/main?un={{ ltrim($item->getText(), '@') }}"
               target="_blank">{{ $item->getText() }}</a>
            @break
        @case (5)
            {{--视频
        ...
        @case (17)
            ...
            @break
        @case (18)
            {{--话题 {"link": "http://tieba.baidu.com/mo/q/hotMessage?topic_id={topic id}&fid={forum id}&topic_name={topic name}", "text": "#{topic name}#", "type": "18"} --}}
